	...
	void RenderHUD()
	{
		...
		if (Collectables::g_tokenCollectionState == Collectables::TOKEN_COLLECTION_STATE_CUTSCENE)
		{
			if (g_isPaused == 0)
			{
				...
				if ((InputManager::g_curButtonsPressed & INPUT_UP) != 0 && (InputManager::g_prevButtonsPressed & INPUT_UP) == 0
					&& g_tokenPromptSelection == 1)
				{
					g_tokenPromptSelection = 0;
					AudioManager::PlaySoundEffect(0x3F, NULL);
		...
		if (g_isPaused != 0 && g_pauseCheatTimer != 0)
		{
			...
			uint32_t colours[12] = { 0, 0, 0, 0, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80 };
			...
			colours[g_pauseMenuSelection] = 0;
			colours[g_pauseMenuSelection + 4] = selectionBrightness;
			colours[g_pauseMenuSelection + 8] = selectionBrightness;
			if (g_pauseMenuState == PAUSE_MENU_MAIN)
			{
				Renderer::DrawString(84, "pause menu", 0x80, 0x80, 0, 0);
				Renderer::DrawString(96, "continue", colours[8], colours[4], colours[0], 0);
				Renderer::DrawString(104, "camera mode", colours[9], colours[5], colours[1], 0);
				Renderer::DrawString(112, "volume control", colours[10], colours[6], colours[2], 0);
				Renderer::DrawString(120, "exit level", colours[11], colours[7], colours[3], 0);
			}
			if (g_pauseMenuState == PAUSE_MENU_CAMERA)
			{
				Renderer::DrawString(84, "camera mode", 0x80, 0x80, 0, 0);
				Renderer::DrawString(100, "passive camera", colours[8], colours[4], colours[0], 0);
				Renderer::DrawString(108, "active camera", colours[9], colours[5], colours[1], 0);
			}
			if (g_pauseMenuState == PAUSE_MENU_VOLUME)
			{
				Renderer::DrawString(84, "volume control", 0x80, 0x80, 0, 0);
				Renderer::DrawString(100, g_pauseSoundVolumeText, colours[8], colours[4], colours[0], 0);
				Renderer::DrawString(108, g_pauseMusicVolumeText, colours[9], colours[5], colours[1], 0);
			}
			if (g_pauseMenuState == PAUSE_MENU_QUIT)
			{
				Renderer::DrawString(84, "are you sure?", 0x80, 0x80, 0, 0);
				Renderer::DrawString(100, "YES", colours[8], colours[4], colours[0], 0);
				Renderer::DrawString(108, "NO", colours[9], colours[5], colours[1], 0);
			}
			if (g_pauseMenuState == PAUSE_MENU_SECRET)
			{
				Renderer::DrawString(84, "pause menu", 0x80, 0x80, 0, 0);
				Renderer::DrawString(100, "continue", colours[8], colours[4], colours[0], 0);
				Renderer::DrawString(108, "exit game", colours[9], colours[5], colours[1], 0);
			}
		...
		if (challengeOffset < 1)
		{
			...
			if (AndysHouse::g_raceCheckpointPassCount < 100)
			{
				...
				{
					uint8_t pulse = g_framePulsePhases.sixtyFourTick;
					if (pulse > 31)
						pulse = 63 - pulse;
					colour = pulse * 4 + 20;
				}
			...
			else
			{
				...
				if (g_specialPickupCount >= 0)
				{
					...
					{
						uint8_t pulse = g_framePulsePhases.thirtyTwoTick & 31;
						if (pulse > 15)
							pulse = 31 - pulse;
						colour = pulse * 4 + 80;
						if (g_framePulsePhases.thirtyTwoTick <= 15)
		...
		if (healthOffset < 1)
		{
			...
			if (healthLength < 5 && g_framePulsePhases.sixteenTick < 8)
				healthLength = 0;
			Renderer::Sprite::DrawScaled(476, (int16_t)healthOffset - (int16_t)healthLength + 47, 6, 0, 0x80, healthLength * 6, 0, 0x60, 0x5000, healthLength << 12);
		...
		int16_t rightColumnX = (int16_t)rightColumnOffset;
		if (objectiveOffset < 1)
		{
			...
			if (g_levelObjectiveProgress == 5)
			{
				...
				uint8_t pulse = (g_framePulsePhases.thirtyTwoTick - 16) & 31;
				if (pulse > 15)
					pulse = 31 - pulse;
				colour = pulse * 4 + 80;
			}
			...
			{
				Renderer::Sprite::DrawScaled(474 - (int16_t)((rightColumnOffset << 9) / 320), (int16_t)objectiveOffset + 32, 20,
					g_levelObjectiveProgress + 26, 0xFF, 0xFF, 0xFF, 0x60, 0x1000, 0x1000);
			}
			Renderer::Sprite::DrawColouredFixed(272 - rightColumnX, (int16_t)objectiveOffset + 16, 50, 0, colour, colour, colour);
		}
		...
		if (objectiveOffset == 1)
			objectiveOffset = -64;
		rightColumnX += (int16_t)((objectiveOffset + 64) / 2);
		if (gadgetOffset < 1)
		{
			uint8_t pulse = (g_framePulsePhases.thirtyTwoTick + 8) & 31;
			if (pulse > 15)
				pulse = 31 - pulse;
			uint8_t colour = pulse * 4 + 80;
			Renderer::Sprite::DrawColouredFixed(272 - rightColumnX, (int16_t)gadgetOffset + 16, 27, 0, colour, colour, colour);
		}
		...
		if (tokenOffset < 1)
		{
			...
			if (pulse > 15)
				pulse = 31 - pulse;
			uint8_t colour = pulse * 4 + 80;
			Renderer::Sprite::DrawColouredFixed(272 - (rightColumnX + (int16_t)((gadgetOffset + 64) / 2)), (int16_t)tokenOffset + 16, 32, 0, colour, colour, colour);
		}
		...
		if (coinsOffset < 1)
		{
			...
			Renderer::Sprite::DrawScaled(63, y, 20, g_buzzActor.coinsCollected % 10 + 26, 0xFF, 0xFF, 0xFF, 0x60, 0x1000, 0x1000);
		}
		...
		if (spinOffset < 1)
		{
			...
			if (g_spinHoverTimer < -119)
				spinScale = -(g_spinHoverTimer + 120) / 6;
			uint32_t red = 0x80;
			uint32_t green = 0x80;
			if (g_framePulsePhases.eightTick < 4 && (g_spinHoverTimer > 59 || g_spinHoverTimer < -119))
			{
				red = 0;
				green = 0;
			}
		...
		if (weaponOffset < 1)
		{
			...
			int16_t leftColumnX = (int16_t)leftColumnOffset;
			Renderer::Sprite::DrawScaledFixed(272 - leftColumnX, yOffset + 224, 15, 0, 0x80, 0x80, 0x80, 0xFF, 0x800, 0x800);

			int16_t ammoYOffset = -7;
			int32_t ammoCount = 0;
			int32_t ammoIcon = 0;
			...
			if (ammoCount != 0)
			{
				...
				Renderer::Sprite::DrawScaledFixed(290 - leftColumnX, ammoY + 216, 20, ammoCount / 10 + 26, 0xFF, 0xFF, 0xFF, 0x60, 0x800, 0x800);
				Renderer::Sprite::DrawScaledFixed(297 - leftColumnX, ammoY + 216, 20, ammoCount % 10 + 26, 0xFF, 0xFF, 0xFF, 0x60, 0x800, 0x800);
				if (ammoIcon == 28)
					Renderer::Sprite::DrawScaled(438 - (int16_t)((leftColumnOffset << 9) / 320), ammoY + 214, 28, 0, 0x80, 0x80, 0x80, 0x60, 0xC00, 0x800);
				else
					Renderer::Sprite::DrawColouredFixed(274 - leftColumnX, ammoY + 206, 33, 0, 0x80, 0x60, 0x40);
			}
			...
			if (ammoYOffset != 0)
			{
				...
				{
					int8_t flashOffset = 0;
					if (g_gunChargeTimer == 64 && g_framePulsePhases.eightTick < 4)
						flashOffset = -128;
					red = (uint8_t)(g_gunChargeTimer * 2 - flashOffset);
					green = (uint8_t)(-128 - flashOffset);
					scale = (g_gunChargeTimer * 0x1644) / 2;
				}
				else
				{
					red = (uint8_t)(g_poweredLaserCharge / 16 + 64);
					green = 0;
					scale = g_poweredLaserCharge * 0x98;
				}
				Renderer::Sprite::DrawScaled(438 - (int16_t)((leftColumnOffset << 9) / 320), yOffset + 218, 6, 0, green, red, 0, 0x60, scale, 0x3000);
				Renderer::Sprite::DrawTiledFixed(272 - leftColumnX, yOffset + 216, 13, 0);
			}
		}
		...
		if (bossOffset <= 0)
		{
			...
			else if (g_hudActorAnimationFrame >= 55)
				g_hudActorAnimationFrame = 54;

			if ((g_hudActorAnimationFrame >= 11 && g_hudActorAnimationFrame < 55) || g_framePulsePhases.sixteenTick < 8)
			{
				Renderer::Sprite::DrawScaled(228, (int16_t)bossOffset + 20, 6, 0, 0x80, (uint8_t)(g_hudActorAnimationFrame * 2), 0, 0x60,
					g_hudActorAnimationFrame << 12, 0x4000);
			}
