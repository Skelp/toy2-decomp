	...
	namespace Particles
	{
		...
		void DrawParticles()
		{
				...
				particle++)
			{
				...
				uvBottomRight.x = ((float)sheet->tileWidth + (float)sheet->tiles[particle->tileIndex].x)
					/ (float)(int32_t)bitmapWidth;
				uvBottomRight.y = ((float)sheet->tileHeight + (float)sheet->tiles[particle->tileIndex].y)
					/ (float)(int32_t)bitmapHeight;
				...
				if ((particleRenderFlags & RENDER_CULL_FRONT) != 0)
				{
					...
					Sprite::QueueGroundAlignedSprite(&position,
						-trigIndex,
						(float)particle->width,
						(float)particle->height,
						&uvTopLeft,
						&uvBottomRight,
						textureDataIndex,
						color,
				...
				{
					particle->renderFlags = particleRenderFlags + RENDER_BILINEAR_FILTER;
					Sprite::QueueQuadSprite(&position,
						trigIndex,
						(float)particle->width,
						(float)particle->height,
						&uvTopLeft,
