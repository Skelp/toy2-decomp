	...
	Light* Light::BuildPoint(const D3DMATRIX* transform, const LightColor* color, float range, int32_t enabled)
	{
		...
		if (light)
		{
			if (DrawingDevice::CreateLight(&light->direct3DLight) == 0)
			{
				...
				light->typedDescription.dltType = D3DLIGHT_POINT;
		...
		int32_t theta, int32_t phi, int32_t enabled)
	{
		...
		if (light)
		{
			if (DrawingDevice::CreateLight(&light->direct3DLight) == 0)
			{
				...
				int32_t active = enabled != 0;
				light->enabled = active;
				light->typedDescription.dltType = D3DLIGHT_SPOT;
				light->typedDescription.dcvColor.r = light->color.r;
				light->typedDescription.dcvColor.g = light->color.g;
				light->typedDescription.dcvColor.b = light->color.b;
				light->typedDescription.dcvColor.a = light->color.a;
				...
				light->typedDescription.dvAttenuation1 = 1.0f;
				light->typedDescription.dvAttenuation2 = 1.0f;
				light->typedDescription.dvTheta = (float)theta * 3.14159265358979323846f / 32768.0f;
				light->typedDescription.dvPhi = (float)phi * 3.14159265358979323846f / 32768.0f;
				light->typedDescription.dwFlags = active;
